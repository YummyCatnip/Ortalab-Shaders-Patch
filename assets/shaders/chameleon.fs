...
extern MY_HIGHP_OR_MEDIUMP vec2 chameleon;
...
vec4 dissolve_mask(vec4 tex, vec2 texture_coords, vec2 uv)
{
    ...
    return vec4(shadow ? vec3(0.0,0.0,0.0) : tex.xyz, res > adjusted_dissolve ? (shadow ? tex.a*0.3: tex.a) : .0);
}
...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    float t = chameleon.g + time;
    // vec4 chame_tex = HSL(tex);
    
    // if (uv.x < 0.20 || uv.x > 0.80 || uv.y < 0.05 || uv.y > 0.95){
    //     chame_tex.a = 0.0;
    // }
    ...
    // if (chameleon.g > 0.0 || chameleon.g < 0.0) {
    //     chame_tex = RGB(chame_tex);
    // }
